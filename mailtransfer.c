...
void usage(char **argv)
{
	fprintf(stderr,
	  ...
	  "  -w, --firmware  Send firmware to Mailstation via parallel port\n"
	  "  -r, --recv      Receive file from Mailstation via parallel port\n\n"
	  "OPTIONS are:\n"
	  "  -h, --help       This help\n"
...
int main(int argc, char **argv)
{
	...
	if (opt_operation == OP_FIRMWARE || opt_operation == OP_SEND) {
		fd = fopen(fpath, "rb");
		if (fd == NULL) {
	...
	if (opt_operation == OP_FIRMWARE) {

		if(filesize != 1024*1024) {
			fprintf(stderr, "Firmware file must be exactly 1 MiB!\n");
			return 1;
		}

		fprintf(stderr, "%d bytes loaded.\nConnect Mailstation to power adapter and press Enter to continue.\n", filesize);
		getchar();

		if (!sendbyte(0x0A)) {
			fprintf(stderr, "FAIL SEND INITIAL COMMAND");
			return 1;
		}
		recvbyte();

		if(!sendbyte(0x05)) {
			fprintf(stderr, "FAIL SEND ERASE COMMAND");
			return 1;
		}
		recvbyte();

		printf("Sending dummy sector...\n");
		sendbyte(0x1B);
		sendbyte(0xFF);
		sendbyte(0xFF);
		sendbyte(0x00);
		...
		sendbyte(0xE7);
		recvbyte();
		printf("Dummy sector sent.\n");
		recvbyte();
		printf("Codeflash is erased.\n");

		for(int sector = 0; sector < 4096; sector ++) {
			uint8_t chksum = 0x19;
			...
			chksum += ((sector >> 8) & 0xFF);
			sendbyte(0x00);
			for(int i = 0; i < 256; i++) {
				sendbyte(fbuf[sector * 256 + i]);
				chksum += fbuf[sector * 256 + i];
			}
			sendbyte(0x100 - chksum);
			recvbyte();
			printf("Sector %d sent, checksum = %08X.\n", sector, 0x100 - chksum);
		}

		printf("Sending last sector...\n");
		sendbyte(0x1B);
		sendbyte(0x00);
		sendbyte(0x00);
		sendbyte(0xFF);
